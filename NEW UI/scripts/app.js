window.addEventListener('DOMContentLoaded', function() {
  ...
  async function fetchLanguages() {
    ...
    try {
      const res = await fetch('https://svhbw2871pdm5w-5000.proxy.runpod.net/get_languages');
  ...
  async function setUserLanguage(user, lang) {
    try {
      await fetch('https://svhbw2871pdm5w-5000.proxy.runpod.net/set_user_language', {
        method: 'POST',
  ...
  async function sendAudioToFullAIWhisper(audioBlob, language = "en", user = 1, userIdx = 0) {
    ...
    try {
      const response = await fetch('https://svhbw2871pdm5w-5000.proxy.runpod.net/fullAI_whisper?user=' + user, {
        method: 'POST',
        body: formData
      });
      if (!response.ok) {
